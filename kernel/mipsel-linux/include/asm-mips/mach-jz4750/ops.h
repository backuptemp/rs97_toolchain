...
/* SSI0 trigger, m = 1, 4, 8, 14 */
#define __ssi0_set_tx_trigger(m)				\
	do {							\
		REG_SSI_CR1(0) &= ~SSI0_CR1_TTRG_MASK;		\
		REG_SSI_CR1(0) |= SSI0_CR1_TTRG_##m;		\
	} while (0)

/* SSI0 trigger, m = 1, 4, 8, 14 */
#define __ssi0_set_rx_trigger(m)				\
	do {							\
		REG_SSI_CR1(0) &= ~SSI0_CR1_RTRG_MASK;		\
		REG_SSI_CR1(0) |= SSI0_CR1_RTRG_##m;		\
	} while (0)

/* SSI1 trigger, m = ix8 */
#define __ssi1_set_tx_trigger(m)				\
	do {							\
		REG_SSI_CR1(1) &= ~SSI1_CR1_TTRG_MASK;		\
		REG_SSI_CR1(1) |= ((m)/8)<<SSI1_CR1_TTRG_BIT;	\
	} while (0)

/* SSI1 trigger, m = ix8 */
#define __ssi1_set_rx_trigger(m)				\
	do {							\
		REG_SSI_CR1(1) &= ~SSI1_CR1_RTRG_MASK;		\
		REG_SSI_CR1(1) |= ((m)/8)<<SSI1_CR1_RTRG_BIT;	\
	} while (0)
...
#if 0
#define __ecc_cnt_dec(n)          ( REG_BCH_CNT = (n) << BCH_CNT_DEC_BIT )
#define __ecc_cnt_enc(n)          ( REG_BCH_CNT = (n) << BCH_CNT_ENC_BIT )
